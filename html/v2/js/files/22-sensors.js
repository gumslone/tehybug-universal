...
(function () {
  ...
  function i2cInner() {
    ...
    const found = I2C.filter(s => d[s[0]]);
  ...
  T.definePage({
    ...
    render() {
      ...
      return html`${UI.pagehead('Sensors', 'Tell the device what is plugged into its ports. I²C sensors are found by themselves.')}
        ...
        <div class="grid-2">
        ${UI.card({ title: 'Port A (black)', icon: 'cpu', body: html`
          ...
          ${UI.choice({ name: 'portA', value: portA, options: [
        ...
        ${display
          ...
          <p class="hint">Shares its pins with the I²C bus: while a DHT or DS18B20 is enabled here, I²C sensors and the clock/data-log module are not available. Prefer Port A for a DHT if you use the data log.</p>
