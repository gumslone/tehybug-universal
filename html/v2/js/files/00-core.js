...
(function () {
  ...
  T.Api = {
    ...
    // restarts the device into its WiFi portal (saved network kept until replaced)
    async wifiPortal() {
      const r = await timeoutFetch('/api/wifiportal', { method: 'POST' }, 8000);
      if (!r.ok) throw new Error(r.status === 302 || r.status === 404 ? 'this firmware has no "change WiFi" support - update it first' : 'HTTP ' + r.status);
      return true;
    },
