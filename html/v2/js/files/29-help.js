    ...
    render() {
      ...
      return html`${UI.pagehead('Help')}
        ${UI.card({ title: 'Troubleshooting', icon: 'help-circle', body: html`
          ...
          ${faq('How do I change the WiFi network?', html`<ul><li><strong>From this page:</strong> <a href="#/system">Power &amp; go live → Change WiFi network</a>. The device restarts into its own network (<code>TEHYBUG-…</code>, password <code>${T.AP_PASSWORD}</code>) with the network chooser at <code>http://192.168.4.1/</code>; nothing else is erased, and <em>Exit</em> keeps the old network.</li><li><strong>When the saved network is gone</strong> (new router, moved house): enter setup mode — RESET, then MODE within a second — and the device opens that same chooser by itself once it cannot join the saved network.</li><li>A factory reset (RESET, then hold MODE ~20 s until the LED turns red) also clears the WiFi, but erases every setting with it — only needed if the device is unreachable.</li></ul>`)}
