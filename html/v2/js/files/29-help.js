      ...
      return html`${UI.pagehead('Help')}
        ...
        ${UI.card({ title: 'Buttons & LED', icon: 'cpu', body: html`
          ${UI.table(['', 'Action', 'Effect'], [
            ...
            [html`<strong>MODE</strong>`, 'press within 1 s after RESET', html`Setup mode — LED <span class="badge info">blue</span>`],
            [html`<strong>MODE</strong>`, 'hold ~20 s after RESET', html`Factory reset — LED <span class="badge warn">red</span>`],
            ...(display ? [[html`<strong>Left / Right</strong>`, 'click', 'Switch screen page; mute an alarm'], [html`<strong>Right</strong>`, 'hold 10 s', html`Toggle offline mode — LED <span class="badge">purple</span>`]] : [])
          ])}
